...
from BGNet.models.feature_extractor_fast import feature_extraction
from BGNet.models.submodules3d import CoeffsPredictor
from BGNet.models.submodules2d import HourglassRefinement
from BGNet.models.submodules import SubModule, convbn_2d_lrelu, convbn_3d_lrelu, convbn_2d_Tanh
import torch
...
class BGNet(SubModule):
    ...
    def forward(self, left_input, right_input):
        ...
        # torch.Size([1, 32, 160, 320]) torch.Size([1, 352, 40, 80])
        _, right_gwc_feature = self.feature_extraction(right_input)

        guide = self.guide(left_low_level_features_1)  # [B,1,H,W] 从左图中得到的指导信息 torch.Size([1, 1, 160, 320])

        cost_volume = build_gwc_volume(left_gwc_feature, right_gwc_feature, 25, 44)  # 最大视差设置为25 分为44组
        # 建立代价体积 torch.Size([1, 44, 25, 40, 80])

        cost_volume = self.dres0(cost_volume)  # 进行代价聚合  torch.Size([1, 16, 25, 40, 80])

        # coeffs:[B,D,G,H,W]
        coeffs = self.coeffs_disparity_predictor(cost_volume) # torch.Size([1, 25, 32, 40, 80])  系数预测，使用了一个残差跳跃,更换了位置
        list_coeffs = torch.split(coeffs, 1, dim=1)  # 分为列表 25个，以视差为单位的列表
        index = torch.arange(0, 97) # 0->96选择97的原因是97到99之间的数都可以
        index_float = index / 4.0  # 除以4，原因是要向上采样4倍tensor([ 0.0000,  0.2500,  0.5000,  0.7500,  1.0000,  1.2500,  1.5000,  1.7500, 2.0000,  2.2500,  2.5000,  2.7500,  3.0000,  3.2500,  3.5000,  3.7500
        index_a = torch.floor(index_float)  # 向下取整 0->24
        index_b = index_a + 1  # 加1  1->25

        index_a = torch.clamp(index_a, min=0, max=24)  # 挤压到0-》24之间
        index_b = torch.clamp(index_b, min=0, max=24)  # 挤压到1-》24之间

        wa = index_b - index_float  # tensor([1.0000, 0.7500, 0.5000, 0.2500, 1.0000, 0.7500, 0.5000, 0.2500, 1.0000,
        wb = index_float - index_a  # tensor([0.0000, 0.2500, 0.5000, 0.7500, 0.0000, 0.2500, 0.5000, 0.7500, 0.0000,
        ...
        # start = time.time()
        for i in range(25):
            slice_dict.append(self.slice(list_coeffs[i], wg, hg, guide))  # [B,1,H,W]  # 先给最大视差这一层扩充
        slice_dict_a = []
        slice_dict_b = []
        for i in range(97):
            inx_a = i // 4
